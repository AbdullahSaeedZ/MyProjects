...
private:
	...
	static string _ReplaceExactWord(string str, string OldWord, string NewWord)
	{
		short pos = 0;
...
public:
	...
	{
		clsDate tempDate = StringToDate(strDate, delim);
	...
	clsDate(short DaysNum, short Year)
	{
		...
		while (true)
		{
			monthDays = NumOfMonthDays(this->_Year, this->_Month);
	...
	// MSVC Non-standard extension for property access.
	...
	static int NumOfYearMinutes(short num)
	{
		return NumOfYearHours(num) * 60;
	}

	int NumOfYearMinutes()
	{
		return NumOfYearMinutes(this->_Year);
	}


	static int NumOfYearSeconds(short num)
	{
		return NumOfYearMinutes(num) * 60;
	}

	int NumOfYearSeconds()
	{
	...
	{
		return (mon < 1 || mon > 12) ? 0 : (mon == 2) ? ((IsLeapYear(year)) ? 29 : 28) : (mon == 4 || mon == 6 || mon == 9 || mon == 11) ? 30 : 31;
	}
	...
	static string DateOfDayNumberInYear(short Year, short DaysNum)
	{
		...
		while (true)
		{
			monthDays = NumOfMonthDays(Date._Year, Date._Month);
	...
	static int DaysDiffOfTwoDates(clsDate Date1, clsDate Date2, bool IncludeEndDay = false)
	{
		int Days = 0;
	...
	{
		string Formatted = "";
		Formatted = _ReplaceExactWord(Format, "dd", to_string(Date._Day));
		Formatted = _ReplaceExactWord(Formatted, "mm", to_string(Date._Month));
		Formatted = _ReplaceExactWord(Formatted, "yyyy", to_string(Date._Year));
